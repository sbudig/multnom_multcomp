...
# m: single integer or vector of integers, cluster size
...
f_mult_sim <- function(m_prop, df_simdat){
  ...
  l_dat_o <- lapply(apply(df_simdatu, 1, function(x)
  ...
  l_dat_a1ar <- lapply(l_dat_o, f_add1allrows)
  l_dat_a1or <- lapply(l_dat_o, f_add1onerow)
  combined_list <- list(l_dat_o, l_dat_a1ar, l_dat_a1or)
  l_dat <- do.call(rbind, combined_list)
  
  v_zerocounts <-
    rep(unlist(sapply(l_dat, f_countzeroes)), times = nlevels(df_simdat$modeltype))
  v_colminimum <-
  ...
  df_simdat <- df_simdat[rep(seq_len(nrow(df_simdat)), each = 3),]
  df_simdat$dataset <- rep(c("original","allrows","onerow"), times = nrow(df_simdat)/3)
  ...
  return(cbind(
    ...
    data.frame(
      ...
      df_simdat$dataset,
      czeroes = v_zerocounts,
      ccolmin = v_colminimum,
      cmfz = countmfz,
      cFWER = countFWER,
      cPower = countPower,
      cgPower = countgPower,
...
f_global_sim <- function(m_iprop, df_simdat) {
  ...
  # if future package is used, parallelizing can be used
  # sometimes gets stuck with a lot of simulation runs
